...
import { deletePartner } from "@/app/actions/admin-site";

export const metadata = { title: "Partenaires" };

export default async function Page() {
  const items = await safe(() => prisma.partner.findMany({ orderBy: { order: "asc" } }), []);

  return (
    <div>
      <AdminPageHeader title="Partenaires" description={`${items.length} partenaire(s)`}>
        <Button asChild>
          ...
          <tr key={r.id} className="hover:bg-surface/50">
            ...
            <td className="px-4 py-3">
              <div className="flex items-center justify-end gap-1">
                ...
                <DeleteButton action={deletePartner.bind(null, r.id)} />
              </div>
            </td>
          </tr>
        ))}
