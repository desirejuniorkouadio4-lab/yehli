...
export const metadata = { title: "Détail bénévole" };

export default async function Page({ params }: { params: { id: string } }) {
  const r = await safe(
    () => prisma.volunteerApplication.findUnique({ where: { id: params.id } }),
    null,
  );
  if (!r) notFound();
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted">Candidature reçue le {formatDate(r.createdAt)}</p>
        </div>
        <StatusSelect type="volunteer" id={r.id} status={r.status} options={statusOptions("volunteer")} />
      </div>

      <div className="grid gap-5 lg:grid-cols-3">
        <div className="rounded-xl border border-border bg-white p-6 shadow-sm lg:col-span-2">
          <h2 className="mb-4 font-bold text-dark">Profil du candidat</h2>
          <dl className="grid gap-5 sm:grid-cols-2">
            ...
            <DetailField label="Disponibilité" value={r.availability} />
            <DetailField label="Domaines d'intérêt" value={r.interests} full />
            <DetailField label="Compétences" value={r.skills} full />
            <DetailField label="Expérience associative" value={r.associativeExperience} full />
        ...
        <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
          ...
          <InternalNoteEditor type="volunteer" id={r.id} note={r.internalNote} />
        </div>
      </div>
    </div>
  );
