...
export const metadata = { title: "Bénévoles" };
...
export default async function Page({
  ...
  const items = await safe(
    () => prisma.volunteerApplication.findMany({ where, orderBy: { createdAt: "desc" }, take: 200 }),
    [],
  );
  const options = statusOptions("volunteer");

  return (
    <div>
      <AdminPageHeader title="Candidatures bénévoles" description={`${items.length} candidature(s)`}>
        <ExportButton type="benevoles" />
      </AdminPageHeader>

      <AdminFilters basePath="/admin/benevoles" statuses={STATUS_SETS.volunteer} active={searchParams.status} />

      <AdminTableShell
        headers={["Nom", "Contact", "Ville", "Date", "Statut", ""]}
        empty="Aucune candidature de bénévolat."
        ...
        {items.map((r) => (
          <tr key={r.id} className="hover:bg-surface/50">
            ...
            <td className="px-4 py-3">
              <StatusSelect type="volunteer" id={r.id} status={r.status} options={options} />
            </td>
            <td className="px-4 py-3 text-right">
              <Link
                href={`/admin/benevoles/${r.id}`}
